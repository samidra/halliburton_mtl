...
export class AllRequestAddUpdateComponent {
  ...
  constructor(private titleService: Title,
    ...
    this.titleService.setTitle('Add/Update All Resources | TestTrack HALLIBURTON');
  }
      ...
      next: (res: any) => {
        ...
        const dialogRef = this.dialog.open(resource, {
          data: {
            ...
            data: res, resource_id: action_type != 'Add resource' ? resource_id : ''
          },
          width: '590px',
  ...
  <div class="container-fluid common_dialog">
    <div class="row">
        <div class="col-12">
            ...
            <form autocomplete="off" class="p-2 pb-0" [formGroup]="resource_form" (ngSubmit)="onSubmit()">
<div class="scroll"> 
                ...
                <div class="form-group">
                    <div class="form_field">
                        <label for="chargeRates">Charge Rate:</label>
                        <input type="text" id="chargeRates" class="form-control" placeholder="Charge Rate"
                            formControlName="chargeRates" style="width: 250px !important;">
                    </div>
                    <div *ngIf="resource_form.get('chargeRates')?.invalid && resource_form.get('chargeRates')?.touched"
                        class="text-danger error">

                        <div *ngIf="resource_form.get('chargeRates')?.hasError('required')">
                            Charge Rate is required.
                        </div>

                        <div *ngIf="resource_form.get('chargeRates')?.hasError('pattern')">
                            Please enter numbers only.
                        </div>

                        <div *ngIf="resource_form.get('chargeRates')?.hasError('min')">
                            Charge Rate must be greater than zero.
                        </div>
                    </div>
                ...
                <div class="toggle_div">
                    <div class="form-check form-switch">
                        ...
                        [checked]="isTestProcedureEnabled"
                        (change)="onCheckboxChange($event)" id="flexSwitchCheckDefault">
                        <label class="form-check-label" for="flexSwitchCheckDefault" >Test Procedure Required
                        </label>
                    </div>
                </div>
...
export class resource {
  ...
  get_resource_details(resource_id: any) {
    this.api_service.get_resource_details(resource_id).subscribe({
      next: (res: any) => {
        ...
        this.approvers_list = this.resource_details.approvers != '' ? this.resource_details.approvers?.split(',') : [];
        this.technician_list = this.resource_details.technician != '' ? this.resource_details.technician?.split(',') : [];
        ...
        this.resource_form.patchValue({
          ...
          chargeRates: this.resource_details.chargeRates,
        });
        this.isTestProcedureEnabled = this.resource_details.testProcedure
  ...
  onSubmit() {
    ...
    if (this.resource_form.valid) {
      ...
      const body = {
        ...
        "chargeRates": this.resource_form.get('chargeRates')?.value,
        "userNames": '',
        "userID": this.user_id,
        "TestProcedure": this.isTestProcedureEnabled
      }
  ...
  isTestProcedureEnabled: boolean = false;
  onCheckboxChange(event: Event) {
    const isChecked = (event.target as HTMLInputElement).checked;
    this.isTestProcedureEnabled = isChecked;
  }
